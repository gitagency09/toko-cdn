...
(function ($) {
    ...
    var HHOptions = {
        ...
        init: function () {
            ...
            this.initProductAutoComplete(body);
            this.initRegionDistrictAutoComplete(body);
            ...
            $('.hh-dropzone', el).each(function () {
                ...
                var hhDropzone = new Dropzone(form.get(0), {
                    ...
                    sending: function (file, xhr, formData) {
                        $('#hh-media-frame-modal .hh-loading').show();
                        formData.append('_token', $('meta[name="csrf-token"]').attr('content'));
                    },

                   transformFile: function(file, done) {

                    if(hh_params.enable_crop == 1){
                        ...
                        confirm.classList.add("crop_confirmBtn");
                        ...
                        editor.appendChild(confirm);
                         ...
                         // Create the cancel button
                            ...
                            cancel.classList.add("crop_cancelBtn");
                        ...
                        if(hh_params.fixed_h_w == 1){
                            var cropper_options = {
                                ...
                                data:{ //define cropbox size
                                    width: hh_params.cropper_width,
                                    height:  hh_params.cropper_height,
                                },
                                viewMode : 1
                            };
                        }else{
                            var cropper_options = {
                                aspectRatio: hh_params.aspectRatio,
                                viewMode : 1
                            };
                        }
                ...
                hhDropzone.on("success", function (file, response) {
                    $('#hh-media-frame-modal .hh-loading').hide();
                ...
                hhDropzone.on('error', function (file, response) {
                    ...
                    if (typeof response.message === 'string') {
                        ...
                        base.alert({ status: 0, message: response});
                    } else {
                        base.alert(response);
                    }
                });
        ...
        mediaRender: function (el, multi, target) {
            ...
            $('body').append('<div id="hh-media-frame-modal" class="modal fade ' + _class + '" tabindex="-1" role="dialog" aria-hidden="true">\n' +
                ...
                '                    <h4 class="modal-title">Libraries<a class="btn btn-info btn-xs waves-effect waves-light ml-1 dash_up_img_btn" data-toggle="collapse" href="#hh-media-add-new" aria-expanded="true">+ UPLOAD IMAGE\n' +
                '                            </a></h4>\n' +
                ...
                '                    </button>\n' +
                '                </div>\n' +
                '                <div class="modal-body"><p class="f_select_alt">Select images to add</p>' +
        ...
        initGoogleMap: function (el) {
            var base = this;
            $('.google-map-content123', el).each(function () {
                var googleEl = $(this),
                    googleMap = googleEl.get(0);
                var parent = $('.field-location');
                ...
                var searchEl = $('.has-google-search'),
                    search = searchEl.get(0);
                    console.log($('.hh-city', parent));
            ...
            $('body').on('click', '.add-list-item', function (ev) {
                ...
                $.post(t.attr('data-url'), data, function (respon) {
                    if (typeof respon == 'object') {
                        ...
                        base.initProductAutoComplete(container);
                        base.initRegionDistrictAutoComplete(container);
                    }
        ...
        initProductAutoComplete: function (el) {

            console.log('called');
            el = (el === '') ? $('body') : el;
            var container = $('.field-search', el);
            var url = $('.field-search', el).data('url');
            container.each(function () {
          
                $('.product_autocomplete', this).each(function () {
                    var parent = $(this);
                    var target = parent.attr("id");
                    var sku = $('.'+target+'-sku');
                    var post_id = $('.'+target+'-post-id');

                    var input = this;
                    // var queryInput = document.querySelector(target);
                              ...
                              success:function(response) {
                                  ...
                                  if(response.status == 1){
                                    $.each(response.data, function(key, value) {
                                        list.push({ label: value.label, value: value.value , id: value.id, sku: value.sku});
                                    }); 
                                  }
                    ...
                    input.addEventListener("awesomplete-selectcomplete", (event) => {
                        input.value = event.text.value.value;
                        console.log(event);
                       
                        // console.log($('.'+target+'-post-id'));
                        // $('.prodshowcase_1_product_1-sku').text(event.text.sku);
                        sku.text(event.text.value.sku);
                        post_id.val(event.text.value.id);
    ...
    initRegionDistrictAutoComplete: function (el) {

            console.log(' initRegionDistrictAutoComplete ');
            el = (el === '') ? $('body') : el;
            var container = $('.field-search_region_district', el);
            var url = $('.field-search_region_district', el).data('url');
            container.each(function () {
          
                //region/district
                $('.region_district_autocomplete', this).each(function () {
                    var parent = $(this);
                    var target = parent.attr("id");
                    var post_id = $('.'+target+'-post-id');
